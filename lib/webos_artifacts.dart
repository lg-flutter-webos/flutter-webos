...
class WebosArtifacts extends CachedArtifacts {
  WebosArtifacts({
  ...
  Directory _getEngineArtifactsDirectory(String arch, BuildMode mode) {
    return _cache.getArtifactDirectory('engine').childDirectory('webos-$arch-${mode.name}');
  }
  ...
  String _getArchForTargetPlatform(TargetPlatform platform) {
    if (platform == TargetPlatform.android_arm64) {
      return 'arm64';
    } else if (platform == TargetPlatform.android_arm) {
      return 'arm';
    } else {
      return 'x64';
    }
