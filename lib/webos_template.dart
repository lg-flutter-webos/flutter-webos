...
class WebosTemplate {
  WebosTemplate._(
    ...
    for (final sourceDirectory in templateSources) {
      if (!sourceDirectory.existsSync()) {
        throwToolExit(
            'WebosTemplate source directory does not exist: ${sourceDirectory.absolute.path}');
      }
    }
    ...
    for (final FileSystemEntity entity in templateFiles.keys.whereType<File>()) {
      ...
      final String relativePath =
          fileSystem.path.relative(entity.path, from: templateFiles[entity]!.absolute.path);
  ...
  static Future<WebosTemplate> fromName(
    String name, {
    required FileSystem fileSystem,
    required Set<Uri>? templateManifest,
    required Logger logger,
  ...
  static Future<WebosTemplate> merged(
    List<String> names,
    Directory directory, {
    required FileSystem fileSystem,
    required Set<Uri> templateManifest,
    ...
    return WebosTemplate._(
      <Directory>[
        for (final String name in names) _templateDirectoryInPackage(name, fileSystem),
      ],
      <Directory>[
  ...
  /// Render the template into [destination].
  ///
  ...
  int render(
    ...
    String? renderPath(String relativeDestinationPath) {
      ...
      if (!relativeDestinationPath.contains('webos') &&
          relativeDestinationPath.contains('web') &&
          !web) {
        return null;
      }
      ...
      if (pluginClassSnakeCase != null) {
        finalDestinationPath =
            finalDestinationPath.replaceAll('pluginClassSnakeCase', pluginClassSnakeCase);
      }
      if (pluginClass != null) {
    ...
    _templateFilePaths.forEach((String relativeDestinationPath, String absoluteSourcePath) {
      ...
      if (sourceFile.path.endsWith(templateExtension)) {
        ...
        // Use a copy of the context, since the original is used in rendering
        // other templates.
...
Directory _templateDirectoryInPackage(String name, FileSystem fileSystem) {
  final String templatesDir =
      fileSystem.path.join(Cache.flutterRoot!, 'packages', 'flutter_tools', 'templates');
