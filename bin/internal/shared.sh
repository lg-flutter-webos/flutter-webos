...
SNAPSHOT_PATH="$ROOT_DIR/bin/cache/flutter-webos.snapshot"
...
function update_flutter() {
  ...
  if [[ ! -f "$stamp_path" || "${version:0:10}" != "$(cat "$stamp_path"| cut -c -10)" ]]; then
    "$FLUTTER_EXE" --version
  fi
}

function update_flutter_webos() {
  mkdir -p "$ROOT_DIR/bin/cache"

  local revision="$(git --git-dir="$ROOT_DIR/.git" rev-parse HEAD)"
  local stamp_path="$ROOT_DIR/bin/cache/flutter-webos.stamp"

  if [[ ! -f "$SNAPSHOT_PATH" || ! -s "$stamp_path" || "$revision" != "$(cat "$stamp_path")"
        || "$ROOT_DIR/pubspec.yaml" -nt "$ROOT_DIR/pubspec.lock" ]]; then

    if [[ "$ROOT_DIR/pubspec.yaml" -nt "$ROOT_DIR/pubspec.lock"
        || "$ROOT_DIR/pubspec.lock" -ot "$FLUTTER_DIR/packages/flutter_tools/pubspec.lock" ]]; then
      echo "Running pub upgrade..."
      (cd "$ROOT_DIR" && "$FLUTTER_EXE" pub upgrade) || {
        >&2 echo "Error: Unable to 'pub upgrade' flutter-webos."
        exit 1
      }
    fi

    echo "Compiling flutter-webos..."
    "$DART_EXE" --disable-dart-dev --no-enable-mirrors \
                --snapshot="$SNAPSHOT_PATH" --packages="$ROOT_DIR/.dart_tool/package_config.json" \
                "$ROOT_DIR/bin/flutter_webos.dart"
